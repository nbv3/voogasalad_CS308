...
import com.syntacticsugar.vooga.util.gui.factory.MenuFactory;
...
public class AuthoringScreenManager implements Observer, IVoogaApp {
	...
	private LevelTabManager myLevelManager;
	...
	public AuthoringScreenManager() {
		...
		myLevelManager = new LevelTabManager(iObject);
		myObjectLibraryManager = new ObjectLibraryManager(myLevelManager);
		initWindow();
	}
	...
	private void initWindow() {
		...
		setUpMyObserver();
		if (myLevelManager.getCurrentLevelEditor() != null) {
			linkObserverAndObservableObjects();
		}
		myWindowGrid.add(myLevelManager.getTabPane(), 0, 0, 1, 2);
	...
	private void buildMenuBar() {
		...
		Menu file = new Menu();
		file.setText(ResourceManager.getString("file"));

		MenuItem newLevel = MenuFactory.buildMenuItem(ResourceManager.getString("new_level"), e -> addLevelRefresh());
		MenuItem loadMap = MenuFactory.buildMenuItem(ResourceManager.getString("load_map"), e -> loadMap());
		MenuItem saveMap = MenuFactory.buildMenuItem(ResourceManager.getString("save_map"), e -> saveMap());
		MenuItem loadData = MenuFactory.buildMenuItem(ResourceManager.getString("load_objectdata"), e -> loadData());
		MenuItem loadGame = MenuFactory.buildMenuItem(ResourceManager.getString("load_game"), e -> loadData());
		MenuItem saveGame = MenuFactory.buildMenuItem(ResourceManager.getString("save_game"), e -> saveGame());
	...
	// Have the authoring environment observe the tower controls, spawner
	// controls, and object libraries
	private void setUpMyObserver() {
		for (int i = 0; i < myLevelManager.getLevels().size(); i++) {
			myLevelManager.getLevels().get(i).getTowerControls().addObserver(this);
			myLevelManager.getLevels().get(i).getSpawnerControls().addObserver(this);
		}
	...
	private void addLevelRefresh() {
		myLevelManager.addNewLevel();
		setUpMyObserver();
	}

	private void saveGame() {
		GameData game = new GameData(myLevelManager.getAllUniverseData(), new GlobalSettings());
	...
	private void loadData() {
		...
		myLevelManager.addNewLevelsFromData(map.getUniverses());

	}

	private void saveMap() {
		MapData toSave = myLevelManager.getIndividualMapData();
		try {
	...
	private void linkObserverAndObservableObjects() {
		Tab pickedLevelTab = myLevelManager.getTabPane().getSelectionModel().getSelectedItem();
		myObjectEditor.addObserver(
				myLevelManager.getCurrentLevelEditor().get(pickedLevelTab).getTowerManager().getTowerView());
		myObjectEditor.addObserver(
				myLevelManager.getCurrentLevelEditor().get(pickedLevelTab).getSpawnerManager().getCurrentView());
	}
